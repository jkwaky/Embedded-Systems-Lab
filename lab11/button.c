...
static void GPIOArm(void){
  GPIO_PORTF_ICR_R = 0xF;      // (e) clear flag4
  GPIO_PORTF_IM_R |= 0xF;      // (f) arm interrupt on PF4 *** No IME bit as mentioned in Book ***
...
void buttonInit(void){
  ...
  GPIO_PORTF_DIR_R &= ~0x0F;     // PF3,PF2,PF1,PF0 inputs
  GPIO_PORTF_DEN_R |= 0x0F;     // enable digital I/O on PF3,PF2,PF1,PF0
	GPIO_PORTF_PUR_R = 0;
  GPIO_PORTF_AFSEL_R &= ~0x0F;  //     disable alt funct on PF3,PF2,PF1,PF0 
  GPIO_PORTF_PCTL_R &= ~0x0000FFFF; // configure PF3, PF2, PF1, PF0 as GPIO
  GPIO_PORTF_AMSEL_R = 0;       //     disable analog functionality on PF
  //GPIO_PORTF_PUR_R |= 0x10;     //     enable weak pull-up on PF4
  GPIO_PORTF_IS_R &= ~0x0F;     // (d) PF4 is edge-sensitive
  GPIO_PORTF_IBE_R |= 0x0F;     //     PF4 is both edges
  GPIOArm();
...
void GPIOPortF_Handler(void){
	GPIO_PORTF_IM_R &= ~0xF; //add and operations
