...
char other[3];
extern int game_check;
extern uint8_t host;
char randomNumber[3]= {'R',0,0};
// Initialize 2
...
void UART_Init(void){
  ...
  UART2_FBRD_R = 27;                     // FBRD = int(0.1267 * 64 + 0.5) = 8
  ...
  GPIO_PORTD_DEN_R |= 0xC0;             // enable digital I/O on PD7-6
                                        // configure PC7-6 as UART
  GPIO_PORTD_PCTL_R = (GPIO_PORTD_PCTL_R&0x00FFFFFF)+0x11000000; //??
  GPIO_PORTD_AMSEL_R = 0;               // disable analog functionality on PD
                                        // UART2=priority 2
  NVIC_PRI8_R = (NVIC_PRI8_R&0xFFFF00FF)|0x00004000; // bits 13-15??
  NVIC_EN1_R |=0x2 ;           // UART2 interrupt??
  //EnableInterrupts();
}



uint8_t getLost(void){
	return lost;
}

void resetGetLost(void){
	lost = 0;
}
void determineField(void){
	char letter;
	int i = 0;
	while(RxFifo_Get(&letter) != FIFOFAIL || i>10){
...
void static copyHardwareToSoftware(void){
  char letter;
  while(((UART2_FR_R&UART_FR_RXFE) == 0) && (RxFifo_Size() < (FIFOSIZE - 1))){
    letter = UART2_DR_R;
    RxFifo_Put(letter);
  }
}
...
void static copySoftwareToHardware(void){
  char letter;
  while(((UART2_FR_R&UART_FR_TXFF) == 0) && (TxFifo_Size() > 0)){
    TxFifo_Get(&letter);
    UART2_DR_R = letter;
  }
}
