...
void PortB_Init(void){
  ...
  GPIO_PORTB_DIR_R &= ~0x0;    // (c) make PB0 in (built-in button)
  ...
  //GPIO_PORTB_PUR_R &= ~0x00;     //     enable weak pull-up on PF4 and PF0
  //GPIO_PORTB_IS_R &= ~0x1;     // (d) PF4 and PF0 is edge-sensitive
  //GPIO_PORTF_IBE_R |= 0x01;     //     PF4 is both edges
	//GPIO_PORTB_IBE_R &= ~0x5;    //     PF4 is not both edges
  //GPIO_PORTB_IEV_R &= ~0x1;    //     PF4 falling edge event
}

void PORTBTOGGLE(void){
